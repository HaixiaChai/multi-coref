...
class CorefModel(nn.Module):
    def __init__(self, config, device, num_genres=None):
        ...
        if config['use_features']:
            self.span_emb_size += 11 * config['feature_emb_size']
        self.pair_emb_size = self.span_emb_size * 3
        if config['use_metadata']:
            self.pair_emb_size += 3 * config['feature_emb_size']
        if config['use_features']:
        ...
        self.emb_span_pos_prior = self.make_embedding(18)
        self.emb_span_deprel_prior = self.make_embedding(38)
        self.emb_span_mention_type_prior = self.make_embedding(5)
        self.emb_span_ud_taxonomy_prior = self.make_embedding(12)
        ...
        self.emb_dt = self.make_embedding(13)
        self.emb_lang = self.make_embedding(10)
        self.emb_word_order = self.make_embedding(2)
        self.emb_pos = self.make_embedding(18)
        self.emb_deprel = self.make_embedding(38)
        self.emb_mention_type = self.make_embedding(5)
        self.emb_ud_taxonomy = self.make_embedding(12)
        ...
        self.span_pos_score_ffnn = self.make_ffnn(config['feature_emb_size'], [config['ffnn_size']] * config['ffnn_depth'], output_size=1)
        self.span_deprel_score_ffnn = self.make_ffnn(config['feature_emb_size'], [config['ffnn_size']] * config['ffnn_depth'], output_size=1)
        self.span_mention_type_score_ffnn = self.make_ffnn(config['feature_emb_size'], [config['ffnn_size']] * config['ffnn_depth'], output_size=1)
        self.span_ud_taxonomy_score_ffnn = self.make_ffnn(config['feature_emb_size'], [config['ffnn_size']] * config['ffnn_depth'], output_size=1)
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map,
                                 is_training, dt, lang, word_order, pos, deprel, head_pos, head_deprel, mention_type, ud_taxonomy,
        ...
        deprel_emb = self.emb_deprel(deprel)
        mention_type_emb = self.emb_mention_type(mention_type)
        ud_taxonomy_emb = self.emb_ud_taxonomy(ud_taxonomy)
        ...
        span_start_pos_emb, span_end_pos_emb = self.dropout(pos_emb[candidate_starts]), self.dropout(pos_emb[candidate_ends])
        span_start_deprel_emb, span_end_deprel_emb = self.dropout(deprel_emb[candidate_starts]), self.dropout(deprel_emb[candidate_ends])
        candidate_emb_list = [span_start_emb, span_end_emb, span_start_pos_emb, span_end_pos_emb, span_start_deprel_emb, span_end_deprel_emb]
        ...
        # pos/deprel emb of head
        head_index = torch.max(candidate_tokens_attn_raw, 1)[1]
        head_pos_emb = self.dropout(pos_emb[head_index])
        candidate_emb_list.append(head_pos_emb)
        head_deprel_emb = self.dropout(deprel_emb[head_index])
        candidate_emb_list.append(head_deprel_emb)
        # avg pos/deprel emb
        token_ones = torch.ones(num_words, dtype=torch.float, device=device)
        candidate_tokens_raw = torch.log(candidate_tokens_mask.to(torch.float)) + torch.unsqueeze(token_ones, 0)
        candidate_tokens_avg = nn.functional.softmax(candidate_tokens_raw, dim=1)
        avg_pos_emb = self.dropout(torch.matmul(candidate_tokens_avg, pos_emb))
        avg_deprel_emb = self.dropout(torch.matmul(candidate_tokens_avg, deprel_emb))
        candidate_emb_list.append(avg_pos_emb)
        candidate_emb_list.append(avg_deprel_emb)
        # add emb of mention type and taxonomy
        head_mention_type_emb = self.dropout(mention_type_emb[head_index])
        candidate_emb_list.append(head_mention_type_emb)
        head_ud_taxonomy_emb = self.dropout(ud_taxonomy_emb[head_index])
        candidate_emb_list.append(head_ud_taxonomy_emb)
        ...
        # head pos/deprel score
        pos_score = torch.squeeze(self.span_pos_score_ffnn(self.emb_span_pos_prior.weight), 1)
        candidate_pos_score = pos_score[pos[head_index]]
        candidate_mention_scores += candidate_pos_score
        deprel_score = torch.squeeze(self.span_deprel_score_ffnn(self.emb_span_deprel_prior.weight), 1)
        candidate_deprel_score = deprel_score[deprel[head_index]]
        candidate_mention_scores += candidate_deprel_score
        # mention type/ud taxonomy score
        mention_type_score = torch.squeeze(self.span_mention_type_score_ffnn(self.emb_span_mention_type_prior.weight), 1)
        candidate_mention_type_score = mention_type_score[mention_type[head_index]]
        candidate_mention_scores += candidate_mention_type_score
        ud_taxonomy_score = torch.squeeze(self.span_ud_taxonomy_score_ffnn(self.emb_span_ud_taxonomy_prior.weight), 1)
        candidate_ud_taxonomy_score = ud_taxonomy_score[ud_taxonomy[head_index]]
        candidate_mention_scores += candidate_ud_taxonomy_score
        ...
        if conf['fine_grained']:
            dt_emb, lang_emb, word_order_emb, seg_distance_emb, top_antecedent_distance_emb = None, None, None, None, None
            if conf['use_metadata']:
                ...
                '''
                dt_emb = self.emb_dt(dt)
                dt_emb = torch.unsqueeze(torch.unsqueeze(dt_emb, 0), 0).repeat(num_top_spans, max_top_antecedents, 1)
                lang_emb = self.emb_dt(lang)
                lang_emb = torch.unsqueeze(torch.unsqueeze(lang_emb, 0), 0).repeat(num_top_spans, max_top_antecedents, 1)
                word_order_emb = self.emb_dt(word_order)
                word_order_emb = torch.unsqueeze(torch.unsqueeze(word_order_emb, 0), 0).repeat(num_top_spans, max_top_antecedents, 1)
            ...
            for depth in range(conf['coref_depth']):
                ...
                if conf['use_metadata']:  # speaker, genre
                    ...
                    feature_list.append(dt_emb)
                    feature_list.append(lang_emb)
                    feature_list.append(word_order_emb)
                if conf['use_segment_distance']:
        ...
        if self.debug:
            if self.update_steps % 20 == 0:
                ...
                logger.info('spans/gold: %d/%d; ratio: %.4f' % (num_top_spans, (top_span_cluster_ids > 0).sum(), (top_span_cluster_ids > 0).sum().tolist()/num_top_spans))
                if conf['mention_loss_coef']:
