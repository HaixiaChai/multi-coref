...
class CorefDataProcessor:
    ...
    @classmethod
    def convert_to_torch_tensor(cls, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map, is_training, 
                                dt, lang, word_order, pos, deprel, head_pos, head_deprel, mention_type, ud_taxonomy,
        ...
        lang = torch.tensor(lang, dtype=torch.long)
        word_order = torch.tensor(word_order, dtype=torch.long)
        pos = torch.tensor(pos, dtype=torch.long)
        deprel = torch.tensor(deprel, dtype=torch.long)
        head_pos = torch.tensor(head_pos, dtype=torch.long)
        head_deprel = torch.tensor(head_deprel, dtype=torch.long)
        mention_type = torch.tensor(mention_type, dtype=torch.long)
        ud_taxonomy = torch.tensor(ud_taxonomy, dtype=torch.long)
...
class Tensorizer:
    ...
    def tensorize_example(self, example, is_training):
        ...
        dt = example['dataset']
        lang = example['lang']
        word_order = example['word_order']
        pos = example['pos']
        deprel = example['deprel']
        head_pos = example['head_pos']
        head_deprel = example['head_deprel']
        mention_type = example['mention_typ']
        ud_taxonomy = example['ud_taxonomy']
        ...
        if is_training and len(sentences) > self.config['max_training_sentences']:
            #self.tmp.append(len(sentences) - self.config['max_training_sentences'])
            return doc_key, self.truncate_example(*example_tensor)
        else:
            #self.tmp_dev.append(len(sentences))
    ...
    def truncate_example(self, input_ids, input_mask, speaker_ids, sentence_len, genre, sentence_map, is_training,
        ...
        pos = pos[word_offset: word_offset + num_words]
        deprel = deprel[word_offset: word_offset + num_words]
        head_pos = head_pos[word_offset: word_offset + num_words]
        head_deprel = head_deprel[word_offset: word_offset + num_words]
        mention_type = mention_type[word_offset: word_offset + num_words]
        ud_taxonomy = ud_taxonomy[word_offset: word_offset + num_words]
