...
class Runner:
    ...
    def train(self, model):
        ...
        # Set up data
        examples_train, examples_dev, examples_tst = self.data.get_tensor_examples()
        ...
        #f1, _ = self.evaluate(model, examples_dev, stored_info, 0, official=False, conll_path=self.config['conll_eval_path'], tb_writer=tb_writer)
            ...
            for doc_key, example in examples_train:
                ...
                if len(loss_during_accum) % grad_accum == 0:
                    ...
                    for scheduler in schedulers:
                        scheduler.step()
                    '''
                    #bert_lr = schedulers[0].get_lr() # my_lr = my_lr_scheduler.optimizer.param_groups[0]['lr']
                    bert_lr = schedulers[0].optimizer.param_groups[0]['lr']
                    task_lr = schedulers[1].optimizer.param_groups[0]['lr']
                    print(type(bert_lr), bert_lr, task_lr)
                    logger.info('bert lr: %.2f' % float(bert_lr))
                    logger.info('task lr: %.2f' % float(task_lr))
                    '''
                    ...
                    if len(loss_history) > 0 and len(loss_history) % conf['eval_frequency'] == 0:
                        ...
                        if f1 > max_f1:
                            max_f1 = f1
                            best_model_path = self.save_model_checkpoint(model, len(loss_history), max_f1)
        ...
        f1, _ = self.evaluate(model, examples_tst, stored_info, len(loss_history), official=False, conll_path=self.config['conll_test_path'], tb_writer=tb_writer)
        if best_model_path is not None:
            ...
            self.evaluate(model, examples_tst, stored_info, len(loss_history), official=False, conll_path=self.config['conll_test_path'], tb_writer=tb_writer)
            #self.evaluate(model, examples_tst, stored_info, 0, official=True, conll_path=self.config['conll_test_path'], save_predictions=join(self.config['log_dir'], self.name_suffix + "_predictions-best.conllu"))
        # Wrap up
        tb_writer.close()
        return loss_history
    ...
    def evaluate(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None, save_predictions=None):
        ...
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            ...
            tensor_example = tensor_example[:16]  # Strip out gold
    ...
    def save_model_checkpoint(self, model, step, f1):
        if step < 30000:
            return  # Debug
...
if __name__ == '__main__':
    config_name, gpu_id = sys.argv[1], int(sys.argv[2]) if len(sys.argv) >=3 else None
